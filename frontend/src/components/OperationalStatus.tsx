...
function StatusCard({ item }: { item: StatusItem }) {
  ...
  return (
    <div
      className={`card card-pad flex items-center gap-4 ${tone.ring}`}
    >
      <div
        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${tone.iconBg}`}
      >
        <Icon
          className={`h-5 w-5 ${tone.iconText}`}
          strokeWidth={2}
        />
      </div>
      ...
      <div className="min-w-0">
        <p
          className={`text-2xl font-semibold tracking-tight ${tone.value}`}
        >
...
export function OperationalStatus({
  ...
  const items: StatusItem[] = [
...

    {
      ...
      value: degradation.degraded_methods,
      icon: CreditCard,
      tone: 'slate',
    },

    {
      ...
      value: degradation.affected_subscriptions,
      icon: Users,
      tone: 'violet',
    },
  ];
    ...
    <section className="flex flex-col gap-4">
      <div>
        ...
        <p className="mt-0.5 text-sm text-ink-500">
          Current workload and system health
        </p>
      </div>
